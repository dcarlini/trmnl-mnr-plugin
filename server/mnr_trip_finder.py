...
class MNR_Trip_Finder:
    ...
    def _periodic_update_gtfs(self, use_local_path):
        while True:
            time_module.sleep(7 * 24 * 60 * 60)  # Sleep for 1 week
            self._update_gtfs_data()
    ...
    def parse_gtfs_time(self, t_str):
        ...
        base_datetime = datetime.combine(date.today(), time(0, 0)) + timedelta(seconds=total_seconds)
        return base_datetime.time(), total_seconds
    ...
    def get_realtime_updates_for_trip(self, realtime_updates, trip_short_name, trip_id, from_id, to_id):
        ...
        return updated_departure_time, updated_arrival_time, departure_delay, arrival_delay

    def find_trips(self, origin, destination, date_str=None):
        target_date = datetime.strptime(date_str, "%Y-%m-%d").date() if date_str else date.today()
        now = datetime.now()
        now_sec = now.hour * 3600 + now.minute * 60 + now.second if target_date == date.today() else 0
        ...
        for trip_id, stops in trip_stops.items():
            ...
            if from_id in stop_ids and to_id in stop_ids and stop_ids.index(from_id) < stop_ids.index(to_id):
                ...
                dep_time_obj, dep_sec = self.parse_gtfs_time(from_stop["departure_time"])
                ...
                duration_minutes = (arr_sec - dep_sec) // 60
                track = from_stop.get("stop_headsign", "") or from_stop.get("track", "") or "N/A"
                departure_time_fmt = dep_time_obj.strftime("%I:%M %p")
                arrival_time_fmt = arr_time_obj.strftime("%I:%M %p")

                trip_short_name = trip_short_name_map.get(trip_id, "")
                updated_departure_time, updated_arrival_time, departure_delay, arrival_delay = self.get_realtime_updates_for_trip(realtime_updates, trip_short_name, trip_id, from_id, to_id)
        ...
        return sorted(upcoming_trips, key=lambda x: x["scheduled_departure_time"])[:10]


if __name__ == "__main__":
    # Basic manual test block
    use_realtime = True
    output_format = "text"
    origin = "Stamford"
    ...
    if output_format == "json":
        ...
        for trip in trips:
            print(f"{trip['scheduled_departure_time']} - {trip['scheduled_arrival_time']} | {trip['origin']} -> {trip['destination']} | Trip ID: {trip['trip_id']} | Trip Short Name: {trip['trip_short_name']} | Track: {trip['track']} | Duration: {trip['duration_minutes']} min | Stops: {trip['stop_count']} | Last Stop: {trip['last_stop']} | Updated Departure: {trip['updated_departure_time']} | Updated Arrival: {trip['updated_arrival_time']} | Dep Delay: {trip['departure_delay']} | Arr Delay: {trip['arrival_delay']}")
